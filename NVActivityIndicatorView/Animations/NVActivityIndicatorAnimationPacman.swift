...
class NVActivityIndicatorAnimationPacman: NVActivityIndicatorAnimationDelegate {
    ...
    func pacmanInLayer(_ layer: CALayer, size: CGSize, color: UIColor) {
        ...
        let y:CGFloat = (layer.bounds.size.height - size.height) / 2 + size.height / 2 - pacmanSize / 2
        let frame = CGRect(
            x: x,
            y: y,
            width: pacmanSize,
    ...
    func circleInLayer(_ layer: CALayer, size: CGSize, color: UIColor) {
        ...
        let x:CGFloat = (layer.bounds.size.width - size.width) / 2 + size.width - circleSize
        let y:CGFloat = (layer.bounds.size.height - size.height) / 2 + size.height / 2 - circleSize
            / 2
        let frame = CGRect(
            x: x,
            y: y,
